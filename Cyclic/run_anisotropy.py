...
networks with uniform strength distribution.
"""
...
def main():
    ...
    if rep_sim_flag:
        ...
        results_comments = "# stretch[0] true[1] nominal[2] fraction_broken[3] G"
    ...
    for i, nub3 in enumerate(nub3_array):
        ...
        for j in range(nPreStretches):
            ...
            if not rep_sim_flag:
                ...
                rep_folder_names_xx = "rep_DNs", "density_effect", "nu" + str(i + 1), "PS" + str( j+ 1), "xx"
                results_file_xx = "data_rep_xx.csv"
                out_xx = sim.runsim_cyclic_rep(geometry_file, model, params, dim, loading_xx, 
                                                stretch_increment, peak_stretches[i][0], 
                                                failure_criterion, data_file, rep_folder_names_xx)
                
                ## Run representative simulation in the y direction
                rep_folder_names_yy = "rep_DNs", "density_effect", "nu" + str(i + 1), "PS" + str( j+ 1), "yy"
                results_file_yy = "data_rep_yy.csv"
                out_yy = sim.runsim_cyclic_anisotropy_rep(model, params, dim, loading_yy, stretch_increment,
                                                            peak_stretches[i][1], out_xx[0][-1], failure_criterion, 
