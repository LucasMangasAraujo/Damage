...
def runsim_frac(geometry_file, model, params, dim, loading, stretch_increment, 
    ...
    while path_exists:
        ...
        ## Print currrent step data
        print("The chain parameters are b = %g, N = %g, rNb_crit = %g" %(computational_bKuhn, NKuhn, critical_r_Nb))
        print("F_11 = %g, F_22 = %g, F_33 = %g" %tuple(F))
        print("S_11 = %g, S_22 = %g, S_33 = %g" %tuple(cauchy_stress))
        print("P_11 = %g, P_22 = %g, P_33 = %g" %tuple(nominal_stress))
        print("G = %g kPa" %G)
    ...
    # Find the non zero stress component based on the loading
    cauchy, nominal = post.nonZero_stress(stretch_array, cauchy_stress_array, 
                                                                nominal_stress_array, loading)
    
    # Assemble data matrix and returhn simulation results
    data_tuple = (np.array(stretch_array), np.array(cauchy), np.array(nominal), 
                    np.array(fraction_broken_chains), np.array(G_array) , 
                    np.array(r0_array) )
    data_matrix = np.column_stack(data_tuple, )
    
    out = data_matrix, np.sqrt(np.mean(preStretch_distr **2))
...
def get_damaged_G(DN, DN_initial, dim, model, computational_bKuhn, bKuhn, loading):
    ...
    # Obtain the damaged end-to-end distance
    rms_r0 = np.mean(DN_damaged.get_rms_r0())
...
def run_reduced_inc(data_file, stretch_increment, loading, dim, main_file, max_attempts = 3):
    """
    ...
    reduced_fail_flag = False ## flag for the case when reducing increment size fails
    ...
    if reduced_fail_flag:
        print("Reduced increments with previous time-step size did not work, trying with reduced time-step size")
        reduce_LAMMPS_timestep(main_file)
        while current_inc < stretch_increment:
            err = runinc(loading, 0, reduced_inc, dim, main_file = "small_step.in")
            if err:
                breakpoint()
            else:
                ## Check if scissions ocurred during the reduced increment
